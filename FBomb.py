...
def spam_handler():
    ...
    phone = parse_phone(input(f"{BRIGHT}{BLUE}FBomb >> {RESET_ALL}"))
    print()
    print("Использовать прокси? (y/n)")
    proxy = input(f"{BRIGHT}{BLUE}FBomb >> {RESET_ALL}")
    if proxy.lower() == "y":
    ...
    print(f"{BRIGHT}{RED}Ctrl+Z для остановки скрипта{RESET_ALL}")
...
def parse_phone(phone):
    ...
    if len(phone) in [10, 11, 12]:
        ...
        print(f"\n{BRIGHT}{RED}[Х] Номер телефона введён неверно!{RESET_ALL}")
        sleep(1)
        spam_handler()
...
def check_internet():
    try:
        ...
        print(f"\n{BRIGHT}{RED}[X] Не удалось подключится к сети, проверьте интернет подключение{RESET_ALL}")
        sleep(1)
        main()
    return
